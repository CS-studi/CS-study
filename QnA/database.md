...
<details>
<summary>Redis는 어떤 경우에 사용할까요?</summary>
    
운영 중인 서버에서 키-값 형태의 데이터 타입을 처리해야하고, I/O가 빈번히 발생해 다른 저장 방식을 사용하면 효율이 떨어지는 경우에 사용합니다. 유튜브 조회수와 같은 카운트 형태의 데이터를 예로 들 수 있습니다. 인기 영상일 경우 RDS 형태의 데이터에 저장해 I/O를 반복한다면 엄청난 자원을 사용할 것입니다. 이를 Redis를 사용해 데이터를 캐싱 처리하고 일정한 주기에 따라 RDS에 업데이트를 한다면 RDS에 가해지는 부담을 크게 줄이고 성능은 크게 향상시킬 수 있을 것입니다.
또 사용자의 세션 관리에도 사용될 수 있는데 사용자의 세션을 유지하고, 불러오고, 여러 활동들을 추적하는데 매우 효과적으로 사용할 수 있습니다.
    
</details>

<details>
<summary>Redis의 장점은 무엇이 있을까요?</summary>
    
먼저 레디스는 In-Memory 데이터베이스로 데이터를 메모리에 저장하고 조회하기 때문에 기존 관계형 데이터베이스보다 빠릅니다.
그리고 다양한 자료구조를 제공해 개발의 편의성을 증가시켜줍니다. 
또 트랜잭션 문제도 해결해 줄 수 있습니다. Redis 자료구조는 Atomic하다는 특징 때문에 race condition을 피할 수 있습니다.
