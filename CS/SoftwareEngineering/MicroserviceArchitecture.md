...
모놀리식에서 마이크로서비스 아키텍쳐로 전환하기 위해서 Application 단에서부터 Infra 까지 수정해야한다. 수정 작업만으로도 매우 부담스럽지만 이후 마이크로 서비스들을 관리하는게 까다롭다. 

하지만 MSA를 도입하므로써 고질적인 문제가 해결될 수 있다. 예를 들면, 배포가 빈번한 기능이 한정되어 있고, 특정 서비스에 scaling out이 필요한 경우 MSA가 좋은 해결책이 될 수 있다.
