...
ex) 1. 키보드로부터 입력을 받는 작업 2. 컴퓨터에서 처리된 결과를 화면에 출력하는 작업 3. 디스크에서 파일 데이터를 읽어오는 작업 등
...
1, 4번째의 스케줄링은 강제로 빼앗지 않고 자진 반납하는 `nonpreemptive` 방식이고, 그 외의 스케줄링은 CPU를 강제로 빼앗는 `preemptive` 방식이다. 3번째 스케줄링은 I/O 작업이 완료된 프로세스가 인터럽트 당한 프로세스보다 우선순위가 높아, 인터럽트 처리 후 직전에 수행되던 프로세스에게 CPU를 다시 할당하는 것이 아니라 문맥교환을 통해 I/O가 완료된 프로세스에게 CPU를 할당하는 경우가 해당한다.
...
- 디스패치 지연시간(dispatch latency): 디스패처가 하나의 프로세스를 정지시키고 다른 프로세스에게 CPU를 전달하기까지 걸리는 시간. 디스패치 지연시간은 문맥교환 오버헤드에 해당된다.
...
- Ready Queue를 우선 순위에 따라 여러 개로 분할한다.
    - 어떤 줄에 서 있는 프로세스를 우선적으로 스케줄링할 것인가, 프로세스를 어느 줄에 세워야할 것인가를 결정해야한다.  
    - 빠른 응답을 필요로 하는 대화형 작업은 전위 큐에 넣는다.
        - 전위 큐에서는 주로 라운드 로빈 스케줄링 기법을 사용한다. (응답시간을 짧게 하기 위해) 
    - 계산 위주의 작업은 후위 큐에 넣는다.
        - 후위 큐에서는 주로 FCFS 스케줄링 기법을 사용한다. (응답시간이 큰 의미를 가지지 않기 때문에 문맥교환 오버헤드를 줄이도록 한다.)
- 멀티 레벨 큐 자체에 대한 스케줄링이 필요하다. (어느 큐에 먼저 CPU를 할당할 것인가)   
